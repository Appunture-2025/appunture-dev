...
\subsection{Sistema de Favoritos}

O \textit{Appunture} oferece um sistema completo de favoritos, permitindo que o usuário personalize sua experiência de estudo e consulta com acesso rápido aos pontos mais relevantes.

\textbf{Funcionalidade de Favoritos:}
\begin{itemize}
    \item \textbf{Adicionar/Remover favoritos}: O usuário pode marcar qualquer ponto de acupuntura como favorito com um único toque no ícone de coração presente nas telas de detalhes e listagem;
    \item \textbf{Lista personalizada}: Todos os pontos favoritados são organizados em uma lista dedicada, acessível pela aba ``Favoritos'' na navegação principal;
    \item \textbf{Acesso rápido}: A lista de favoritos permite consulta imediata dos pontos mais utilizados, sem necessidade de busca;
    \item \textbf{Gestão simplificada}: Cada item da lista possui botão para remoção rápida dos favoritos;
    \item \textbf{Sincronização na nuvem}: Os favoritos são sincronizados com o \textit{backend} quando o usuário está autenticado, permitindo acesso em múltiplos dispositivos;
    \item \textbf{Optimistic Updates}: A interface atualiza imediatamente ao favoritar/desfavoritar, com rollback automático em caso de falha de sincronização.
\end{itemize}

Esta funcionalidade garante que o usuário não perca seus dados ao trocar de dispositivo, mantendo sua experiência personalizada sempre disponível.
