...
\subsection{Processamento de Linguagem Natural e Assistente IA}

Para viabilizar a busca inteligente por sintomas, o aplicativo implementa dois níveis de processamento:

\textbf{Busca Local (Offline):}
O módulo local utiliza técnicas de normalização textual, remoção de \textit{stopwords} e busca difusa (\textit{fuzzy search}) com a biblioteca Fuse.js. Este sistema permite interpretar entradas do usuário em linguagem natural, incluindo correções automáticas de erros de digitação, e mapear sintomas a pontos de acupuntura clinicamente indicados.

\textbf{Assistente IA com RAG (Online):}
Quando conectado à internet, o sistema utiliza a arquitetura RAG (\textit{Retrieval-Augmented Generation}) integrada ao \textit{backend}. O assistente inteligente, alimentado pelo Google Gemini 1.5 Flash via Spring AI, interpreta perguntas complexas sobre sintomas e tratamentos, fornecendo respostas contextualizadas com a base de dados técnica do sistema. Esta abordagem reduz alucinações do modelo e aumenta a confiabilidade das sugestões clínicas.
...
O sistema de busca do \textit{Appunture} utiliza técnicas de processamento de linguagem natural para oferecer resultados relevantes mesmo quando o usuário não conhece o nome exato do ponto que procura. A busca pode ser realizada por:
...
O algoritmo de busca difusa (\textit{fuzzy search}) implementado com Fuse.js permite encontrar resultados mesmo com erros de digitação ou variações na escrita.
