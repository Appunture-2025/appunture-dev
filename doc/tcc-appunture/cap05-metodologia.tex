...
\begin{enumerate}
    ...
    \item Como usuário, desejo filtrar pontos com base em sintomas que estou tratando (ex: ``ansiedade'' + ``insônia''), para encontrar sugestões apropriadas.
    \begin{itemize}
        \item Critérios de aceitação: Deve aceitar múltiplos sintomas. Deve retornar pontos relacionados por frequência de uso clínico.
    \end{itemize}
    ...
    \item Como usuário, desejo usar um assistente inteligente para descrever meus sintomas com linguagem natural, para receber recomendações automáticas baseadas em evidências.
    \begin{itemize}
        \item Critérios de aceitação: Quando \textit{online}, deve utilizar IA Generativa (Spring AI + Google Gemini) para sugestões contextualizadas. Respostas devem ser baseadas na base de dados técnica do sistema.
    \end{itemize}
    
    \item Como administrador, desejo acessar o painel web, para visualizar e gerenciar os dados do aplicativo.
    \begin{itemize}
        \item Critérios de aceitação: Login restrito ao perfil administrador. Interface adaptada para \textit{desktop}.
    \end{itemize}
    
    \item Como administrador, desejo cadastrar e editar pontos de acupuntura, para manter o banco de dados atualizado.
    \begin{itemize}
        \item Critérios de aceitação: Deve permitir adicionar nome, descrição, localização, meridiano, imagem etc. Interface amigável com validação dos campos.
    \end{itemize}
    
    \item Como administrador, desejo consultar relatórios de uso, para entender como os usuários estão utilizando a plataforma.
    \begin{itemize}
        \item Critérios de aceitação: Relatórios por data, localização, pontos mais acessados. Visualização em gráfico ou tabela.
    \end{itemize}
\end{enumerate}
...
\hline
14 & CRUD completo de pontos e sintomas (admin) & Web/Backend & 6 & Alta \\
\hline
15 & Upload de imagens SVG e definição de coordenadas & Web Admin & 3 & Média \\
\hline
16 & Controle de acesso por perfil (user/admin) & Todas & 3 & Alta \\
\hline
...
\hline
Painel administrativo para cadastro e upload de imagens SVG & 4 \\
\hline
...
\hline
Sprint 7 & 12--13 & 12 & Backend completo + painel administrativo com upload de SVG \\
\hline
